...
- **Match the guest to the screen rather than stretching it.** Stretching blurs text and wastes space on letterbox bars — and, as below, it also breaks the pointer. The guest can be told to change mode (VBE makes this possible), so the honest approach is a small **menu of resolutions baked into the image** (720p, 1280×800, 1440×900, 1080p, …) with the page picking the best fit.
- **Whole-number scaling is a requirement, because the pointer depends on it.** v86 reports mouse movement as raw host-pixel deltas and never divides by the display scale, so a screen shown at anything other than 1:1 moves the guest's cursor at the wrong rate — shrunk it lags your hand, enlarged it runs ahead and reaches the screen's edge before your hand does, which is how controls near the edges become unclickable. The gap compounds. Measured, not theorised.
- **So there are two honest answers: 1:1, or capture the pointer.** At 1:1 the pointer is exact. With the pointer captured, the host cursor is hidden and there is no second cursor left to disagree with. A guest that enables the **VMware absolute pointer** escapes the rule entirely — v86 sends absolute coordinates too, but only uses them when the guest opts in, and stock TinyCore's X has no vmmouse driver. A known-good mouse driver in our own image therefore removes a whole class of display problems, not just this one.
- **When no whole number fits, say so instead of quietly drifting.** The prototype labels the scale and offers capture; silence would be the worst option. The durable fix is a **guest mode that fits the screen** (a small resolution menu in the image), so 1:1 is usually available — distro work, not page work.
- **Readable text is two problems, not one.** Resampling is ours: integer scales with nearest-neighbour keep glyphs crisp, while a fractional fit turns every glyph to mush. The font itself belongs to the image: stock TinyCore paints unantialiased 6×13 bitmap fonts, and our own image ships a real font stack instead. Until then, showing a small guest mode at 2× is the cheap trick — bigger *and* sharper.
